...
# Функция, возвращающая безопасную строку
# 'conninfo' для подключения к PostgreSQL
def build_pg_coninfo(
        db_name: str,
        host: str,
        port: int,
        user: str,
...
# Функция, логирующая версию СУБД, к которой происходит подкчлючение
...
async def get_pg_connection(
    ...
    conninfo = build_pg_coninfo(db_name, host, port, user, password)
    connection: AsyncConnection | None = None

    try:
        connection = await AsyncConnection.connection(conninfo=conninfo)
