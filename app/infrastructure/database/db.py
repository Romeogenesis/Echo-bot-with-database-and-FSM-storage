    ...
    async with conn.cursor() as cursor:
        await cursor.execute(
            ...
            params={
                ...
                "username": username,
                "language": language,
                "role": role.value,  # Предполагаем, что UserRole — Enum
                "is_alive": is_alive,
                "banned": banned,
            },
        )
    logger.info(
        "User added. user_id=%d, username='%s', language='%s', role=%s, is_alive=%s, banned=%s",
        user_id,
        username,
        language,
        role,
...
async def update_user_lang(
    ...
    *,
    ...
    logger.info("the language '%s' is set for user '%s'", language, user_id)
...
async def get_user_alive_status(
    ...
    *,
    ...
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("the user with 'user_id'=%s has the alive_status %s", user_id, row[0])
    else:
        logger.warning("no user with 'user_id'=%s found in the database", user_id)
    return row[0] if row else None
