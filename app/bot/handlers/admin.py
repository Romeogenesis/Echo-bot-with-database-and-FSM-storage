...
# Этот хэндлер будет срабатывать на команду /help для 
# пользоватея со статусом UserRole.ADMIN
@admin_router.message(Command("help"))
async def process_admin_help_command(message: Message, i18n: dict):
    await message.answer(text=i18n.get('/help_admin'))

# Этот хэндлер будет срабатывать на команду /statistics для 
# пользователя с ролью UserRole.ADMIN
...
async def process_admin_statistics_command(
    ...
    await message.answer(
        text=i18n.get("statistics").format(
            "/n".join(
                f"{i}. <b>{start[0]}</b>: {start[1]}"
                for i, start in enumerate(statistics, 1)
            )
        )
    )

# Этот хэндлер будет срабатывать на команду /ban для пользователя 
# с ролью UserRole ADMIN
